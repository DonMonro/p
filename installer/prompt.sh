...
run_prompt() {
    ...
    # credentials are now BAKED INTO the panel wheel (panel/psiphon/__init__.py
    # ships _PUBLIC_* constants extracted from the public Psiphon-3 client).
    # Per-country tunnels establish out-of-the-box — no install-time survey
    # step. A commercial sponsor who needs its own PropChannel / SponsorId /
    # signed server-list URL / sig-pubkey can still set the PSIPHON_* env
    # vars in /opt/psiphon-3x-ui/panel.env AFTER install (overrides the
    # defaults at render_config-time). See docs/TROUBLESHOOTING.md.
