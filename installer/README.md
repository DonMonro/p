...
|------|--------------------|-----------------|
| [`deps.sh`](deps.sh) | `run_deps` | apt install: python3 (venv/pip/build), golang-go, git, jq, curl, wget, tar, ca-certificates. **Not** `ufw` — Phase 29 dropped host-firewall management, and an already-installed ufw is left untouched |
...
| [`panel_install.sh`](panel_install.sh) | `run_panel_install` | venv, build wheel, `panel.seed` the DB, write `panel.env`, register systemd unit (defensively guards that the user/group already exists), install the polkit rule |
