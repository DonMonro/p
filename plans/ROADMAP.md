...
The per-country routing was originally implemented (Hotfix #9) by **direct
config-file edit** of `/usr/local/x-ui/bin/config.json`'s **`outbounds[]`**
+ **`routing.rules[]`** arrays from inside `panel/dashboard/router.py`.
That design was operationally verified to fail in production: the live
`config.json` is owned `root:root` with mode 0600 on a stock 3x-ui install
AND is periodically regenerated by 3x-ui from `/etc/x-ui/x-ui.db`, so the
unprivileged panel service user could neither read nor write it — every
Hotfix-#9 apply/remove call returned EACCES and surfaced an empty
`routing_result`.

**Hotfix #10 (Phase 25) replaces the direct edit with a queue+applier
sidecar:**
...
* systemd-side: `psiphon-xray-applier.path` watches the queue dir and
  batches bursts via `TriggerLimitIntervalSec`; `psiphon-xray-applier.service`
  (`Type=oneshot`, `User=root`) execs
  `/usr/local/libexec/psiphon-3x-ui/xray-applier.sh` which flock()s a
  lockfile, delegates each patch file to the stdlib-only
  `xray_apply.py` merger, and runs `systemctl restart x-ui.service` ONCE
  at the end of the batch (only if at least one patch mutated the config).
* polkit rule extended to let the panel user `systemctl start
  psiphon-xray-applier.service` (belt-and-braces alongside path-unit
  auto-trigger).
